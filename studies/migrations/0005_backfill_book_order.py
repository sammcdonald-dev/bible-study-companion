...
def populate_book_order(apps, schema_editor):
    Verse = apps.get_model("studies", "Verse")
    for book_name, order in BOOK_ORDER.items():
        Verse.objects.filter(book=book_name).update(book_order=order)


class Migration(migrations.Migration):
    dependencies = [
        ("studies", "0004_add_book_order"),
    ]

    operations = [
        migrations.RunPython(populate_book_order, migrations.RunPython.noop),
    ]
